...
**Client project folders are separate** (the work, not the system): `cloudskin-v56`, `mykonos-prestige`, `greencleaners`, `unicorn-tattoo`, `dionyssos-hotel`, `drip-*`, etc. Bring per-project via `git clone` (most are deployed/git-backed); some carry big node_modules so `du` over all of them times out.
