...
(() => {
  ...
  async function getList() {
    ...
    if (base) {
      try {
        // Fetch EVERY post (paginated, 100 per request) so numbered pagination
        // can page through the full archive - not just the first screenful.
        const per = 100;
        ...
        while (page <= totalPages) {
          ...
          if (!Array.isArray(arr)) throw new Error("WP returned an invalid response");
          all = all.concat(arr);
          page++;
          if (arr.length < per) break;
        }
        // A reachable WP with a genuinely empty feed is real (empty), not a fallback
        // trigger; renderIndex already shows a clean "no stories yet" state for it.
        _listCache = { posts: all.map(fromWP), sample: false };
        return _listCache;
      } catch (e) {
  ...
  async function renderIndex(root) {
    ...
    root.innerHTML = filtersHTML + featuredHTML + gridHeadHTML +
      '<div class="jgrid" id="jGrid"></div>' +
      '<nav class="jpager" id="jPager" aria-label="' + esc(t("blog.pager")) + '" hidden></nav>';
    ...
    const pager = document.getElementById("jPager");

    // ---------- numbered pagination (page 1, 2, 3...) ----------
    // Featured is shown on page 1 only, in ADDITION to PER_PAGE grid cards.
    const PER_PAGE = Math.max(1, Number(cfg.perPage) || 9);
    let curList = rest, curFeatured = true, curPage = 1;
    const pageCount = (list) => Math.max(1, Math.ceil(list.length / PER_PAGE));

    // windowed pager markup: ‹ 1 … 4 5 6 … 20 ›
    function buildPager(page, pages) {
      ...
      let html = '<button class="jpager__nav" type="button" data-pg="' + (page - 1) + '"' +
        (page === 1 ? " disabled" : "") + ' aria-label="' + esc(t("blog.prev")) + '">&lsaquo;</button>';
      let prev = 0;
      nums.forEach((n) => {
        if (prev && n - prev > 1) html += '<span class="jpager__gap" aria-hidden="true">&hellip;</span>';
        html += '<button class="jpager__pg' + (n === page ? " is-active" : "") + '" type="button" data-pg="' + n + '"' +
          (n === page ? ' aria-current="page"' : "") +
          ' aria-label="' + esc(fill("blog.page", { n: n })) + '">' + n + "</button>";
        prev = n;
      });
      html += '<button class="jpager__nav" type="button" data-pg="' + (page + 1) + '"' +
        (page === pages ? " disabled" : "") + ' aria-label="' + esc(t("blog.next")) + '">&rsaquo;</button>';
      pager.innerHTML = html;
      pager.querySelectorAll("[data-pg]").forEach((b) => {
        b.addEventListener("click", () => {
          const n = Number(b.dataset.pg);
          if (n >= 1 && n <= pages && n !== curPage) goPage(n, true);
        });
      });
    }

    function renderPage(scrollUp) {
      const pages = pageCount(curList);
      if (curPage > pages) curPage = pages;
      if (curPage < 1) curPage = 1;
      const showFeat = curFeatured && curPage === 1;
      if (feat) feat.hidden = !showFeat;
      const start = (curPage - 1) * PER_PAGE;
      grid.innerHTML = curList.slice(start, start + PER_PAGE).map(cardHTML).join("");
      if (countEl) {
        const cn = curList.length + (curFeatured ? 1 : 0);
        countEl.textContent = fill(cn === 1 ? "blog.countOne" : "blog.count", { n: cn });
      }
      buildPager(curPage, pages);
      reveal(grid.querySelectorAll(".bcard"));
      if (showFeat && feat && feat.classList.contains("reveal")) reveal([feat]);
      if (scrollUp) {
        const anchor = document.getElementById("jSecHead") || grid;
        const y = anchor.getBoundingClientRect().top + window.pageYOffset - 90;
        window.scrollTo({ top: Math.max(0, y), behavior: reduce ? "auto" : "smooth" });
      }
    }

    function syncURL(n, push) {
      const q = new URLSearchParams(location.search);
      if (n <= 1) q.delete("page"); else q.set("page", n);
    ...
    function goPage(n, push) { curPage = n; renderPage(true); syncURL(n, push); }

    function setList(list, showFeatured) {
      curList = list; curFeatured = showFeatured; curPage = 1;
      renderPage(false);
    }

    // initial page from ?page= (deep link / refresh)
    curPage = Math.max(1, parseInt(new URLSearchParams(location.search).get("page"), 10) || 1);
    renderPage(false);

    // browser back / forward between pages
    window.addEventListener("popstate", (e) => {
      const n = (e.state && e.state.blogPage) ||
        Math.max(1, parseInt(new URLSearchParams(location.search).get("page"), 10) || 1);
      curPage = Math.min(Math.max(1, n), pageCount(curList));
      renderPage(false);
    });

    // client-side category filtering (resets pagination to page 1)
    root.querySelectorAll(".jfilter__btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (c === "all") setList(rest, true);
        else setList(posts.filter((p) => p.catSlug === c), false);
        syncURL(1, false);
      });
    });
