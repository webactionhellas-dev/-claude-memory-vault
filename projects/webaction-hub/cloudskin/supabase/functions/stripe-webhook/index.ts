...
// 2026-07-28 REFACTOR (PayPal build): the Shopify-order-creation + shipping-
// recovery logic that used to live inline here is now _shared/fulfillment.ts's
// fulfillPaidOrder(), the SAME function the PayPal capture/webhook path calls.
// This is the concrete fix for the drift the house hit once: this file used to
// bundle its OWN divergent copy of _shared/shopify.ts (send_receipt:true) while
// create-checkout-session's bundle had a different, unused one (send_receipt:
// false) — see _shared/shopify.ts's header. There is now exactly one
// createShopifyOrder, called from exactly one place (fulfillment.ts), for both
// rails. Behavior here is UNCHANGED (same shipping-recovery math, same
// send_receipt/send_fulfillment_receipt:true, same shopify_sync_status
// bookkeeping) — this is a pure extraction.
import { fulfillPaidOrder } from '../_shared/fulfillment.ts';
...
async function markPaidAndSync(
  ...
  // --- Shopify fulfilment sync: the SAME code path the PayPal rail uses ---
