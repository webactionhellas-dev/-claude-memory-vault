...
(() => {
  ...
  const LS_LANG = "cloudskin_lang", LS_CUR = "cloudskin_ccy", SS_IPCUR = "cs_ip_ccy";
  const get = k => { try { return localStorage.getItem(k); } catch (e) { return null; } };
  const set = (k, v) => { try { localStorage.setItem(k, v); } catch (e) {} };
  const getS = k => { try { return sessionStorage.getItem(k); } catch (e) { return null; } };
  const setS = (k, v) => { try { sessionStorage.setItem(k, v); } catch (e) {} };
  ...
  function detectCur() {
    // Timezone reflects the device's LOCATION, so it is a better currency signal
    // than the browser language (an Australian on an en-US phone must still see A$).
    const tz = (Intl.DateTimeFormat().resolvedOptions().timeZone || "").toLowerCase();
    for (const [re, c] of TZ_CUR) if (re.test(tz)) return c;
    const r = regionCode(); if (REGION_CUR[r]) return REGION_CUR[r];
    return "EUR";
  }
  // Authoritative: Vercel gives us the visitor's real country server-side (by IP).
  // Runs once per session, ONLY when the visitor has not manually chosen a currency;
  // reloads a single time if the IP currency differs from the synchronous guess so
  // every rendered price updates. A manual choice (LS_CUR) always wins. If geo is
  // unavailable (localhost / header missing) it keeps the timezone guess, no reload.
  function ipDetectCurrency() {
    try { if (get(LS_CUR) || getS(SS_IPCUR)) return; } catch (e) { return; }
    fetch("/api/geo", { headers: { accept: "application/json" } })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (d) {
        var cc = d && typeof d.country === "string" ? d.country.toUpperCase() : "";
        if (!cc) { setS(SS_IPCUR, C.currency); return; }
        var detected = REGION_CUR[cc] || "EUR";
        setS(SS_IPCUR, detected);
        if (detected !== C.currency) location.reload();
      })
      .catch(function () {});
  }
  ...
  let cur = get(LS_CUR);                            // 1. manual choice persists (localStorage)
  if (!CUR_CODES.includes(cur)) cur = getS(SS_IPCUR); // 2. IP choice resolved earlier this session
  if (!CUR_CODES.includes(cur)) cur = detectCur();   // 3. synchronous best-guess (timezone-first)
  const meta = LANGS.find(l => l.code === lang) || LANGS[0];

  C.lang = lang; C.dir = meta.dir; C.currency = cur; C.LANGS = LANGS; C.CUR = CUR;
  ipDetectCurrency();  // authoritative IP-based currency override (once per session, async)
