...
(() => {
  "use strict";
  // bfcache self-heal: a phone restored from back-forward cache serves a frozen snapshot of the
  // page (stale catalogue photos instead of fresh uploads); force a fresh load on bfcache restore.
    ...
    if (row && pairsBox) {
      if (pairs.length) {
        ...
        row.innerHTML = pairs.map(x => {
          ...
          <span class="pairitem__img"><img src="${C.baseImageFor(x, pcol)}" alt="${x.title}" loading="lazy"></span>
  ...
  function renderWish() {
    ...
    b.innerHTML = list.map(p => `<div class="ditem" data-wishkey="${p.handle}">
      ...
      <div class="ditem__info"><h4><a href="${C.pdpHref(p)}">${p.title}</a></h4><p>${p.color}</p>
        <div class="ditem__row"><button class="wish__add" data-wishadd="${p.handle}">${C.t("pdp.addToBag")}</button>
  ...
  async function checkout() {
    ...
    // Custom Stripe checkout (feature-flagged in js/checkout.js). When enabled it
    // takes over and charges in the customer's currency; otherwise the existing
    // Shopify hosted-checkout path below runs unchanged. Live site is unaffected
    // until the flag is flipped at go-live.
    if (C.checkoutStripe && C.checkoutStripe.enabled()) { C.checkoutStripe.begin(); return; }
    const btns = [$("#checkoutBtn"), $("#addBagCheckout")];
    const setBusy = (b) => btns.forEach(x => { if (x) { x.disabled = b; x.classList.toggle("is-busy", b); } });
    setBusy(true);
    try {
      if (C.shopify && C.shopify.ready) await C.shopify.ready;
      const lines = []; let missing = 0;
      cart.forEach(it => {
        const p = byHandle(it.handle);
        const vid = C.shopify ? C.shopify.variantId(it.handle, it.color || (p && p.color), it.size) : null;
        if (vid) lines.push({ merchandiseId: vid, quantity: it.qty });
        else missing++;
      });
      if (lines.length && C.shopify) {
        const url = await C.shopify.checkout(lines);
        if (url) {
          if (missing) console.warn("[CloudSkin] " + missing + " cart item(s) not yet in Shopify were skipped at checkout.");
          window.location.href = url;   // → Shopify-hosted checkout (real payment/orders)
          return;
        }
      }
      fallbackConfirm();   // Shopify unreachable → never dead-end the customer
    } catch (err) {
      console.warn("[CloudSkin] Shopify checkout failed; showing fallback confirmation.", err);
      fallbackConfirm();
    } finally {
      setBusy(false);
    }
  ...
  C.availColorsFor = (p) => {
    ...
    return colorsAll.filter((c, k) => k < Math.max(1, distinctShots.length));
  };
  ...
  C.baseImageFor = (p, forceColor = null) => {
    ...
    if (p && !p._studioImgs && p.handle && window.CLOUDSKIN_CONTENT_SNAPSHOT) {
      ...
      for (const k in snap) {
        ...
        const urls = String(snap[k] || "").split(/\r?\n/).map(s => s.trim()).filter(Boolean);
  ...
  C.cardHTML = (p, i = 0, forceColor = null, single = false) => {
    ...
    // No hover image-swap: the card shows ONLY photo #1 (static), so there is never a second image
    // to flash or bleed behind it on hover. object-fit:cover fills the fixed 3/4 frame (top-biased
    // focal in main.css) so owner lifestyle photos never letterbox.
    return `
    <article class="pcard" data-handle="${p.handle}" data-color="${displayColor}" style="transition-delay:${(i % 4) * 60}ms">
      <a class="pcard__media" href="${cardHref}" aria-label="${p.title}">
        ${badge}
        <img src="${mainImg}" alt="${p.title} in ${displayColor}" loading="lazy">
      ...
      <button class="pcard__cloud" data-cloudadd="${p.handle}" aria-label="Add ${p.title} to bag" title="Add to bag">
  ...
  /* card colour swatches: swap the card's front image to the chosen colourway.
     Studio (/shopify/) shots pair to colours by index, so swatch i -> shot i. Capture phase so we
     cancel the wrapping product link before it navigates. */
  function swatchPick(sw) {
    const card = sw.closest(".pcard"); if (!card) return;
    const p = byHandle(card.dataset.handle); if (!p) return;
    // Resolve the colour's image the SAME way the card's photo #1 and the PDP do: the owner's Studio
    // per-colour photo first (C.studioImagesForColour, via C.baseImageFor), else the catalogue /shopify/
    // shot. Previously this used ONLY the catalogue shot-by-index, so a product whose catalogue shots are
    // all identical (e.g. the court skirt's white product shots) showed no change on tap and ignored
    // Larissa's uploaded per-colour model photos. Now the card swatch matches the PDP exactly.
    const shot = C.baseImageFor(p, sw.dataset.color);
    const front = card.querySelector(".pcard__media img");
    if (front && shot) front.src = shot;
    card.querySelectorAll(".pcard__swatches i").forEach(x => x.classList.remove("on"));
    sw.classList.add("on");
    card.dataset.color = sw.dataset.color;   // keep the card's colourway in sync so add-to-bag uses the shown colour
    const meta = card.querySelector(".pcard__meta"); if (meta) meta.textContent = sw.dataset.color;
  }
  document.addEventListener("click", e => {
