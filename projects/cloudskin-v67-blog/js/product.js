...
(() => {
  ...
  (function seedStudioFromSnapshot() {
    ...
    Object.keys(snap).forEach((k) => {
      ...
      if (s && urls.length) studioColourImages[s] = urls;
    });
    const flat = snap[flatKey];
    if (flat) {
      const fu = String(flat).split(/\r?\n/).map((x) => x.trim()).filter(Boolean);
      if (fu.length) studioImages = fu;
    }
  })();
  ...
  $("#pdpMain").innerHTML = `
    <div class="gallery${gallery.length < 2 ? " gallery--single" : ""}" id="gallery">
      <div class="gallery__thumbs" id="thumbs">
        ${gallery.map((g, i) => `<button class="${i === 0 ? "on" : ""}" data-i="${i}" aria-label="View image ${i + 1}"><img src="${g}" alt="${p.title} view ${i + 1}" loading="lazy" decoding="async"></button>`).join("")}
      </div>
      <div class="gallery__main" id="galMain"><img id="galImg" src="${gallery[0]}" alt="${p.title} in ${p.color}" decoding="async"></div>
    </div>
  ...
  function setMainImg(url) {
    ...
    if (galImg.getAttribute("src") === url || galImg.src === abs) return;
    galImg.src = url;
  }
  function bindThumbs() {
    $$("#thumbs button").forEach(b => b.addEventListener("click", () => {
      $$("#thumbs button").forEach(x => x.classList.remove("on")); b.classList.add("on");
      galImg.src = gallery[+b.dataset.i];
    }));
  }
  function renderGallery() {
    const gEl = $("#gallery"); if (gEl) gEl.classList.toggle("gallery--single", gallery.length < 2);
    $("#thumbs").innerHTML = gallery.map((g, i) => `<button class="${i === 0 ? "on" : ""}" data-i="${i}" aria-label="View image ${i + 1}"><img src="${g}" alt="${p.title} view ${i + 1}" loading="lazy" decoding="async"></button>`).join("");
  ...
  galMain.addEventListener("mouseleave", () => { galImg.style.transformOrigin = "center center"; });
  ...
  const fill = (id, list) => {
    ...
    if (_snap) list.forEach(pr => {
      ...
      Object.keys(_snap).forEach(k => {
        ...
        const urls = String(_snap[k] || "").split(/\r?\n/).map(s => s.trim()).filter(Boolean);
  ...
  (() => {
    ...
    function applyOverrides(map) {
      ...
      Object.keys(map).forEach((k) => {
        ...
        const urls = String(map[k] || "").split(/\r?\n/).map(s => s.trim()).filter(Boolean);
        if (slug && urls.length) colourMap[slug] = urls;
      });
      studioColourImages = colourMap;
      const imgsRaw = g(flatKey);
      if (imgsRaw) {
        const urls = imgsRaw.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
        if (urls.length) studioImages = urls;
      }
