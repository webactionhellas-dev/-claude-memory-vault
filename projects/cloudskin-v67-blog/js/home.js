...
(() => {
  ...
  function fillRail(id, list, instant) {
    ...
    // Colourway split: show each real colourway as its own card (both colours visible, zero taps),
    // matching the collection grid. Variant-safe - entries come only from real Shopify variants.
    const cards = C.colorwayCards(list);
    el.innerHTML = cards.map((e, i) => C.cardHTML(e.p, i, e.color, true)).join("");
  ...
  if (!reduce && !noParallax) {
    ...
    function onScroll() {
      ...
      // hero parallax: only the IMAGE drifts; the wordmark, sub and buttons stay put
      const y = scrollY;
      if (heroMedia && y < vh * 1.15) {
