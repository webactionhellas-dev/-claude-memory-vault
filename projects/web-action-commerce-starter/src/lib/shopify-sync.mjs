...
export const VENDOR_MAP = {
  ...
  'UGG': 'UGG', 'ACME': 'Acme Exclusive', 'Acme Exclusive': 'Acme Exclusive',
};
export const TYPE_MAP = {
  'Products': 'products', 'Clothing': 'clothing', 'Clothes': 'clothing', 'Tops': 'clothing',
  'Accesorries': 'accessories', 'Accessories': 'accessories', '': 'products',
};
...
export async function fetchAllShopify(storeUrl) {
  ...
  for (let page = 1; page < 50; page++) {
    const res = await fetch(`${base}/products.json?limit=250&page=${page}`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (acme-sync)' },
    });
...
async function samplePlate(url) {
  ...
  const res = await fetch(`${url}${sep}width=120`, { headers: { 'User-Agent': 'Mozilla/5.0 (acme-sync)' } });
 ...
 */
export async function syncShopifyCatalog(sb, { storeUrl = 'https://acme-store.example', write = false, log = () => {}, sampleImages = true, maxSamples = Infinity } = {}) {
  ...
  for (const p of shopify) {
    ...
    const baseCat = TYPE_MAP[p.product_type] ?? 'products';
